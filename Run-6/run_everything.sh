...
###############################################################################
#                       NETRA-ADAPT: RUN-6 PIPELINE                         #
#                                                                             #
#  Five innovations over Run-5:                                              #
#  1. ProtoStyleBank: K=256 FIFO style anchors per class (vs ~3 in Run-5)   #
#  2. Bank warmup: 3 epochs no-grad to seed bank before adaptation           #
#  3. L_con: augmentation consistency loss (KL weak vs strong aug)           #
#  4. L_psl: progressive self-training from epoch 10 (conf > 0.90)          #
...
RESULTS_DIR="$BASE_DIR/results_run6"
TOTAL_START=$(date +%s)
...
###############################################################################
clear
print_header "NETRA-ADAPT RUN-6: PROTO-STYLEBANK + CONSISTENCY + SELF-TRAINING"
echo -e "${BOLD}Date:${NC} $(date '+%Y-%m-%d %H:%M:%S')"
echo -e "${BOLD}Workspace:${NC} $BASE_DIR"
echo -e "${BOLD}Results dir:${NC} $RESULTS_DIR"
echo ""
echo -e "${YELLOW}Run-6 innovations over Run-5:${NC}"
echo -e "  1. ${BOLD}ProtoStyleBank${NC}  — K=256 FIFO style anchors/class across all batches (vs ~3 from batch)"
echo -e "  2. ${BOLD}Bank warmup${NC}     — 3 epochs no-grad to seed bank before any gradient updates"
echo -e "  3. ${BOLD}L_con${NC}           — KL(sg(p_weak)||p_strong) augmentation consistency loss, ramp over 5ep"
echo -e "  4. ${BOLD}L_psl${NC}           — Pseudo-label supervised loss from epoch 10 (conf > 0.90)"
echo -e "  5. ${BOLD}Cosine LR + clip${NC}— CosineAnnealingLR + gradient clipping max_norm=1.0"
echo ""
...
# ── Final Summary ─────────────────────────────────────────────────────────────
print_header "NETRA-ADAPT RUN-6 — PIPELINE COMPLETE"
