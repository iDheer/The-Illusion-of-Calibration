"""
run_full_pipeline.py - Run-6: Full Netra-Adapt Pipeline

Run-6 changes:
- All result paths point to /workspace/results_run6/
- PrototypeMixEnt-Adapt with ProtoStyleBank + consistency + self-training
- Banner updated to reflect Run-6
...
def main():
    print("\n" + "="*80)
    print("   NETRA-ADAPT RUN-6: FULL EXPERIMENTAL PIPELINE")
    ...
    print("\n" + "="*80)
    print("   PIPELINE COMPLETE — RUN-6 SUMMARY")
    print("="*80)
    for step, info in results.items():
        status = "✅" if info["success"] else "❌"
        print(f"  {status} {step:<20} {info['time']/60:.1f} min")
    print(f"\n  Total time: {total_time/60:.1f} minutes")
    print(f"  Results: /workspace/results_run6/evaluation/results_table.csv")
    print("="*80)
