...
SAVE_DIR = "/workspace/results_run7/Oracle_Chaksu"
...
def train():
    ...
    # Run-7: log class breakdown for both train and val subsets to verify
    # there are glaucoma cases in BOTH splits (critical for valid AUROC).
    # In Run-5 val had very few positives which made AUROC unreliable.
    df_full_raw = pd.read_csv(CSV_PATH)
    df_full_raw = df_full_raw[df_full_raw['label'] >= 0].reset_index(drop=True)
    train_labels = [int(df_full_raw.iloc[i]['label']) for i in train_indices]
    val_labels   = [int(df_full_raw.iloc[i]['label']) for i in val_indices]
    tr_g = train_labels.count(1); tr_n = train_labels.count(0)
    vl_g = val_labels.count(1);   vl_n = val_labels.count(0)
    print(f"\n  ┌─ Chákṣu Oracle split breakdown ─────────────────────────")
    print(f"  │  Train: {len(train_labels):>4}  Normal:{tr_n:>4}  Glaucoma:{tr_g:>4}")
    print(f"  │  Val:   {len(val_labels):>4}  Normal:{vl_n:>4}  Glaucoma:{vl_g:>4}")
    if vl_g == 0:
        print(f"  │  ⚠ CRITICAL: 0 glaucoma in val split — AUROC monitoring broken!")
        print(f"  │    This means chaksu_train_labeled.csv has too few glaucoma.")
        print(f"  │    Re-run prepare_data.py and check label matching.")
    elif vl_g < 3:
        print(f"  │  ⚠ WARNING: only {vl_g} glaucoma in val — AUROC score will be noisy")
    else:
        print(f"  │  ✓ Val has {vl_g} glaucoma cases — AUROC monitoring is valid")
    if tr_g == 0:
        print(f"  │  ⚠ CRITICAL: 0 glaucoma in train — model cannot learn!")
    print(f"  │  Backbone: DINOv3 ViT-L/16, last 2 blocks + head trainable")
    print(f"  │  Batch:    {BATCH_SIZE}  Epochs: {MAX_EPOCHS}  LR: backbone=2e-5, head=2e-3")
    print(f"  └───────────────────────────────────────────────")
