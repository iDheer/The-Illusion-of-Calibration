...
###############################################################################
#                       NETRA-ADAPT: RUN-7 PIPELINE                         #
#                                                                             #
#  Run-6 innovations (ProtoStyleBank, warmup, L_con, cosine LR) PLUS:       #
#  Fix A: L_div applied to uncertain samples only (was full batch in Run-6)  #
#  Fix B: L_psl disabled (reinforced mode collapse under domain shift)       #
#  New:   L_balance class-prior KL regulariser [0.85, 0.15]                 #
#                                                                             #
...
RESULTS_DIR="$BASE_DIR/results_run7"
TOTAL_START=$(date +%s)
...
###############################################################################
clear
print_header "NETRA-ADAPT RUN-7: FIXED IM LOSS + PRIOR MATCHING"
echo -e "${BOLD}Date:${NC} $(date '+%Y-%m-%d %H:%M:%S')"
echo -e "${BOLD}Workspace:${NC} $BASE_DIR"
echo -e "${BOLD}Results dir:${NC} $RESULTS_DIR"
echo ""
echo -e "${YELLOW}Run-7 changes over Run-6:${NC}"
echo -e "  Fix A: ${BOLD}L_div scope${NC}    — applied to uncertain samples only (was full batch)"
echo -e "  Fix B: ${BOLD}L_psl disabled${NC} — reinforced mode collapse under domain shift"
echo -e "  New:   ${BOLD}L_balance${NC}      — class-prior KL [0.85, 0.15] prevents drift"
echo ""
...
# ── Final Summary ─────────────────────────────────────────────────────────────
print_header "NETRA-ADAPT RUN-7 — PIPELINE COMPLETE"
