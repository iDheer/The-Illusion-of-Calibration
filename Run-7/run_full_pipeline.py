"""
run_full_pipeline.py - Run-7: Full Netra-Adapt Pipeline

Run-7 changes:
- Fixed L_div scope: applied to uncertain samples only (was full batch in Run-7)
- L_psl disabled: reinforced mode collapse under domain shift
- L_balance added: class-prior KL regulariser [0.85, 0.15]
- All result paths point to /workspace/results_run7/
...
def main():
    print("\n" + "="*80)
    print("   NETRA-ADAPT RUN-7: FULL EXPERIMENTAL PIPELINE")
    print("   Cross-Ethnic Glaucoma Screening — Fixed IM Loss + Prior Matching")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("   PIPELINE COMPLETE — RUN-7 SUMMARY")
    print("="*80)
    for step, info in results.items():
        status = "✅" if info["success"] else "❌"
        print(f"  {status} {step:<20} {info['time']/60:.1f} min")
    print(f"\n  Total time: {total_time/60:.1f} minutes")
    print(f"  Results: /workspace/results_run7/evaluation/results_table.csv")
    print("="*80)
