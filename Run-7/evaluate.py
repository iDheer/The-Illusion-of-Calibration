"""
evaluate.py - Run-7: Comprehensive Model Evaluation for Netra-Adapt

Run-7 changes:
  - Result paths updated to /workspace/results_run7/
  ...
  - prepare_data.py label-matching bug fixed: Run-5 only matched Bosch images
    ...
    useless. Run-7 fix ensures Remidio + Forus labels are also matched, giving
...
RESULTS_DIR     = "/workspace/results_run7/evaluation"

MODELS_CHAKSU = {
    "Pretrained → Chákṣu":     None,
    "AIROGS → Chákṣu":        "/workspace/results_run7/Source_AIROGS/model.pth",
    "Chákṣu → Chákṣu":        "/workspace/results_run7/Oracle_Chaksu/oracle_model.pth",
    "AIROGS+Adapt → Chákṣu":  "/workspace/results_run7/Netra_Adapt/adapted_model.pth",
}

MODELS_AIROGS = {
    "AIROGS → AIROGS": "/workspace/results_run7/Source_AIROGS/model.pth",
}
...
def plot_roc_curves(all_results):
    ...
    plt.title('ROC Curves — Run-7 (PrototypeMixEnt + Consistency + Self-Training)', fontsize=15, fontweight='bold')
...
def plot_metrics_comparison(all_results):
    ...
    plt.suptitle('Performance Metrics — Run-7 (PrototypeMixEnt + Consistency + Self-Training)', fontsize=15,
...
def main():
    ...
    print("\n" + "="*70)
    print("   NETRA-ADAPT Run-7: COMPREHENSIVE EVALUATION")
    ...
    if os.path.exists(TEST_CSV_CHAKSU):
        ...
        if _n_pos < 5:
            ...
            print(f"     Check prepare_data.py label matching (Run-7 fix should give 20+ positives)")
        else:
