...
def parse_chaksu_labels():
    ...
    # Run-7: log ALL columns in each CSV + which img_col/dec_col was selected.
    ...
    for csv_file in csv_files:
        try:
            ...
            for col in df.columns:
                col_lower = col.lower()
                # Run-7 FIX: use FIRST column with 'image' in name (not last).
            ...
            for idx, row in df.iterrows():
                ...
                # Run-7 FIX (extended): register base filenames for compound
                # CSV keys like:
            ...
            if extra_img_note:
                print(f"         NOTE: multiple 'image' cols — Run-7 picks FIRST.{extra_img_note}")
    ...
    # Run-7: track per-device counts and compare against KNOWN_CHAKSU sizes.
    ...
    # Run-7: per-device breakdown reveals instantly if one device has 0% match.
    ...
    # Run-7: Final sanity check comparing test glaucoma count vs expectation
